  ...
  return (
    <Card className={`p-8 ${className}`}>
      <div className="text-center mb-8">
        ...
        <div 
          className="text-lg mb-4 break-all"
      ...
      <ChartContainer
        config={chartConfig}
        className="mx-auto aspect-square max-h-[350px] mb-8"
      >
      ...
      <div className="grid grid-cols-2 gap-4">
        {(['discovery', 'understanding', 'actions', 'trust'] as const).map((categoryKey) => {
          ...
          const passingCount = applicableIndicators.filter(indicator => {
            const status = indicator.evidence?.status || 
              (indicator.applicability.status === 'not_applicable' ? 'not_applicable' :
